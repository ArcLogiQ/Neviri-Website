...
function formatPrice(value, currency = "USD", decimals = 2) {
  const amount = currency === "INR" ? value * USD_TO_INR : value;
...
function formatDelta(val, currency = "USD") {
  if (val === 0) return formatPrice(0, currency);

  const formatted = formatPrice(Math.abs(val), currency);
...
function PricingRow({ vm, billing, currency }) {
    ...
    <tr className="hover:bg-sky-50/50 transition-colors duration-150 relative">
            ...
            <div>
              ...
              <div className="flex flex-col gap-3">
                <div className="relative flex items-center border border-gray-200 rounded-xl overflow-hidden focus-within:border-[#278be1] focus-within:ring-1 focus-within:ring-[#278be1] transition-all">
                  ...
                  />
                  <span className="absolute right-4 text-gray-400 text-sm font-bold pointer-events-none">
                    GB
                  </span>
                </div>
                <button
      ...
      <td className="px-6 py-4 text-right">
        <span className="text-lg font-extrabold text-[#0F172A]">
          {formatPrice(
            billing === "Monthly" ? totalMo : totalHr,
            currency,
            billing === "Monthly" ? 2 : 3,
          )}
        </span>
      ...
      <header className="relative pt-16 pb-12 text-center">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-6">
          ...
          <p className="text-base text-slate-500 max-w-xl mx-auto leading-relaxed mb-6">
            Pay only for what you use. Transparent billing with no surprises —
            hourly or monthly.
          </p>
          <div className="flex items-center justify-center gap-3 flex-wrap">
            {[
              ...
              <span
                key={b}
                className="inline-flex items-center gap-1.5 px-3 py-1 bg-white border border-gray-200 rounded-full text-xs font-semibold text-gray-600 shadow-sm"
              >
          ...
          <div className="flex items-center justify-center mt-6">
            <div className="inline-flex items-center gap-1 p-1 bg-white border border-gray-200 rounded-xl shadow-sm">
              {["USD", "INR"].map((currencyOption) => (
                <button
                  key={currencyOption}
                  type="button"
        ...
        <div className="max-w-7xl mx-auto px-6 flex gap-1 overflow-x-auto">
          {TABS.map(({ label, icon: Icon }) => (
            <button
              ...
              className={`relative flex items-center gap-2 px-5 py-4 text-sm font-semibold whitespace-nowrap transition-all duration-200 cursor-pointer ${
                activeTab === label
                  ? "text-sky-600"
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 md:px-8 py-10 space-y-16 pb-24">
          ...
          <section>
            <div className="mb-6">
              ...
              <p className="text-sm text-gray-500">
                First 25 GB SSD is free with every VM. Adjust storage to see
                total cost.
              </p>
            </div>
                  ...
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200 rounded-t-2xl">
                      {["PLAN", "vCPU", "RAM", "DISK", "PRICE"].map((h, i) => (
                        <th
                          key={h}
                          className={`px-6 py-4 text-[11px] font-bold tracking-widest text-gray-400 uppercase ${i === 0 ? "text-left" : i === 4 ? "text-right" : "text-center"}`}
                        >
                          {h}
                        </th>
                      ))}
        ...
        {activeTab === "Networking" && (
          <section className="space-y-8">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Inbound is always free. 100 GB outbound included every month.
              </p>
            </div>
              ...
              {[
                {
                  ...
                  value: "Free",
                  sub: "Always free, unlimited",
                  color: "green",
                },
                {
                  label: "First 100 GB Outbound",
                  value: "Free",
                  sub: "Per month",
                  color: "green",
                },
                {
                  ...
                  value: formatPrice(0.05, currency),
                  sub: "Per GB beyond 100 GB/mo",
                  color: "sky",
                },
              ].map((item) => (
                <div
                  key={item.label}
                  className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:border-sky-200 hover:shadow-md transition-all duration-200"
                >
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">
                    {item.label}
                  </p>
                  <p
                    className={`text-3xl font-extrabold mb-1 ${item.color === "green" ? "text-green-500" : "text-sky-600"}`}
                  >
                    {item.value}
                  </p>
                    ...
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        {[
                          ...
                          <th
                            key={h}
                            className={`px-6 py-4 text-[11px] font-bold tracking-widest uppercase ${i === 0 ? "text-left text-gray-400" : i === 1 ? "text-center text-sky-600" : "text-center text-gray-400"}`}
                          >
                            {h}
                          </th>
                        ))}
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {RESERVED_IP.map((row) => (
                        ...
                        >
                          ...
                          {[row.neviri, row.do, row.aws, row.azure].map(
                            (val, i) => (
                              <td key={i} className="px-6 py-4 text-center">
                                {val === "Free" ? (
                                  <span className="inline-flex items-center gap-1 px-3 py-1 bg-green-50 text-green-600 text-xs font-bold rounded-full border border-green-200">
                                    <CheckCircle2 className="h-3 w-3" /> Free
                                  </span>
                                ) : (
                                  <span
                                    className={`font-bold text-sm ${i === 0 ? "text-sky-700" : "text-gray-500"}`}
                                  >
                                    {formatPrice(
                                      val,
                                      currency,
        ...
        {activeTab === "Storage & Security" && (
          <section className="space-y-8">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Block storage, automated backups, and SSL — all built in.
              </p>
            </div>
                ...
                <div className="flex items-center justify-between p-4 rounded-xl bg-sky-50 border border-sky-100">
                  ...
                  <div className="text-right flex items-center gap-2">
                    <span className="text-xs font-medium text-sky-600 bg-white px-2 py-0.5 rounded border border-sky-200">
                      {formatPrice(0.00011, currency, 5)} / hr
                    </span>

                    <span className="font-extrabold text-sky-700 text-xl">
                      {formatPrice(0.08, currency)}
                ...
                <div className="space-y-3">
                  {[
                    ...
                    >
                      ...
                      <p className="font-extrabold text-sky-700 text-lg">
              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  ...
                  <p className="text-sky-100 text-sm max-w-md leading-relaxed">
                    Distribute traffic across your VMs and secure every
                    connection with auto-provisioned SSL — zero configuration
                    required.
                  </p>
                </div>
                <div className="flex flex-col gap-2.5 shrink-0">
                  {["SSL Certificate", "Auto-renewal", "Load Balancer"].map(
                    (f) => (
                      <span
                        key={f}
        ...
        {activeTab === "Bandwidth Analysis" && (
          <section className="space-y-6">
            <div>
              <h2 className="text-2xl font-bold text-[#0F172A] mb-1">
                Monthly Bandwidth Cost Comparison
              </h2>
              <p className="text-sm text-gray-500">
                Neviri vs DigitalOcean, AWS, and Akamai across usage tiers
              </p>
            </div>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {BANDWIDTH_COMPARISON.map((row) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5 text-right font-extrabold text-sky-700">
                          {formatPrice(row.neviri, currency)}
                        </td>

                        <td className="px-5 py-3.5 text-right text-gray-500">
                          {formatPrice(row.do, currency)}
                        </td>

                        <td className="px-5 py-3.5 text-right text-gray-500">
                          {formatPrice(row.aws, currency)}
                        </td>

                        <td className="px-5 py-3.5 text-right text-gray-500">
                          {formatPrice(row.akamai, currency)}
                        </td>
                        ...
                        >
                          {formatDelta(row.neviriVsDo, currency)}
                        </td>
                        ...
                        >
                          {formatDelta(row.neviriVsAws, currency)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            {[
              {
                ...
                a: "Yes. Change your billing preference at any time from your dashboard with no penalty.",
              },
              {
                q: "Are there any setup or hidden fees?",
                a: "No. You only pay for the resources you use. All pricing shown is fully inclusive.",
              },
              {
                q: "What happens if I exceed my bandwidth?",
                a: `Outbound traffic beyond 100 GB/mo is billed at ${formatPrice(0.05, currency)}/GB. Inbound is always free.`,
              },
              {
                q: "Can I scale resources up or down?",
                a: "Yes. Resize compute, storage, and networking instantly with zero downtime.",
              },
              {
                q: "Can I migrate my existing workloads to Neviri?",
                a: "Yes. You can migrate websites, applications, and workloads with minimal downtime.",
              },
              {
                q: "Where are Neviri data centers located?",
                a: "Neviri operates data centers in multiple locations for high availability and low latency.",
              },
            ].map((item) => (
              <div
                key={item.q}
                className="bg-white border border-gray-200 rounded-2xl p-5 hover:border-sky-200 hover:shadow-sm transition-all duration-200"
              >
                <div className="flex items-start gap-3">
                  <ChevronRight className="h-4 w-4 text-sky-500 mt-0.5 shrink-0" />
                  <div>
                    <p className="font-semibold text-[#0F172A] text-sm mb-1.5">
                      {item.q}
                    </p>

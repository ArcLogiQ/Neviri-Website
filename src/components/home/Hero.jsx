...
import { ArrowRight, Server, Database, Network, Shield, Activity } from "lucide-react";

const Hero = () => {
  return (
    <section className="relative bg-white min-h-[90vh] flex flex-col pt-32 pb-16 overflow-hidden font-sans antialiased selection:bg-[#3B82F6]/30 selection:text-black">
          ...
          <div className="lg:col-span-7 flex flex-col items-start text-left">
           ...
           <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold text-[#0F172A] mb-8 tracking-[-0.04em] leading-[1.0]">
            ...
            <span className="relative inline-block mt-2">
              Secure, Compliant
              <div className="absolute bottom-1 left-0 w-full h-[6px] bg-[#3B82F6] opacity-50 -z-10"></div>
            </span> <br/>
            Infrastructure.
          </h1>

            <p className="text-lg text-slate-500 mb-10 max-w-md leading-relaxed font-medium">
              Build, deploy, and manage virtual machines, managed databases, and secure networking from one platform with effortless precision.
            </p>
            ...
            <div className="w-[380px] bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-[0_32px_64px_-15px_rgba(0,0,0,0.08)] relative overflow-hidden group hover:-translate-y-2 transition-all duration-500">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-black text-slate-400 uppercase tracking-tighter">System Status</p>
                    <p className="text-sm font-bold text-slate-900">Live Infrastructure</p>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div className="h-[1px] w-full bg-slate-100 relative">
                    <div className="absolute top-0 left-0 h-full bg-[#3B82F6] w-2/3 shadow-[0_0_8px_#3B82F6]"></div>
                </div>
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 grid grid-cols-2 gap-4">
                <div className="flex flex-col">
                  ...
                  <span className="text-sm font-black text-slate-900">99.998%</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Latency</span>
                  <span className="text-sm font-black text-slate-900">14ms</span>
                </div>
              </div>
            </div>
          </div>
...
const Feature = ({ icon, text }) => (
  <div className="flex items-center gap-3 group cursor-default">
    <div className="w-8 h-8 flex items-center justify-center bg-slate-50 border border-slate-200 rounded-lg group-hover:bg-[#3B82F6] group-hover:border-[#3B82F6] transition-all">
      {React.cloneElement(icon, { size: 14, strokeWidth: 3, className: "text-slate-900 group-hover:text-white transition-colors" })}
    </div>
    <span className="font-bold text-[#0F172A] text-lg tracking-tight">{text}</span>
  </div>
);
